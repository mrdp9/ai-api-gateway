...
async function fetchTunnelUrl() {
    try {
        // Construct URL without credentials to avoid browser fetch restrictions
        const baseUrl = `${window.location.protocol}//${window.location.host}`;
        const response = await fetch(`${baseUrl}/api/tunnel-url`);
...
async function deleteApiKey(keyId) {
    ...
    try {
        // Construct URL without credentials to avoid browser fetch restrictions
        const baseUrl = `${window.location.protocol}//${window.location.host}`;
        const response = await fetch(`${baseUrl}/api/delete/${keyId}`, {
            method: 'DELETE'
        });
